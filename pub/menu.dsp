   ...
   function loadDocument() {
    ...
    function refreshDSP() {
      ...
      if (hideStandard == "false" && hideSOAP == "false") {
        window.location = "JMS_trigger.dsp?type=all&$alias=OMPTRG_IS_97"+appendStrAmp;
      }else if (hideStandard == "false") {
        window.location = "JMS_trigger.dsp?type=standard&$alias=OMPTRG_IS_97"+appendStrAmp;
      }else if (hideSOAP == "false") {
        window.location = "JMS_trigger.dsp?type=soap&$alias=OMPTRG_IS_97"+appendStrAmp;
      }else {
        window.location = "JMS_trigger.dsp?$alias=OMPTRG_IS_97"+appendStrQue;
      }
    }
...
<table width="100%">  
          ...
          <tr name="RServer" >
            ...
            %loop -struct servers% 			
                <tr name="RServer" >
                  <script>writeTD("row-l");</script><A id="a%value alias encode(htmlattr)%" TARGET="%ifvar target%%value $host encode(htmlattr)%%value target encode(htmlattr)%%else%body%endif%" HREF="JMS_trigger.dsp?remote_server=%value alias encode(htmlattr)%"> %value alias encode(html)% </a>   </td>
